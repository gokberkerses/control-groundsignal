...
int main(int argc, char **argv)
{
    ...
    ros::Subscriber state_sub_1 = nh.subscribe<mavros_msgs::State>
                                ("/uav2/mavros/state", 10, state_cb_1);
    ...
    ros::Subscriber local_sub_1 = nh.subscribe<nav_msgs::Odometry>
                                ("/uav2/mavros/global_position/local", 10, local_cb_1);                                                                  
    ...
    while (ros::ok()) {
        ...
        if (current_state_1.mode != "OFFBOARD" && !set_offboard_1 &&
                (ros::Time::now() - last_request_1 > ros::Duration(5.0)) && current_state_1.mode != "AUTO.LAND"){// && took_off == true) {
        ...
        //     if( set_mode_client_1.call(takeoff_mode) &&
        ...
        if(mission_end_1 == false)// && took_off == true)
        {
	        local_pos_pub_1.publish(pose_1);
	        ROS_INFO("UAV2 setpoint: %.1f, %.1f, %.1f", pose_1.pose.position.x, pose_1.pose.position.y, pose_1.pose.position.z);
            ROS_INFO("UAV2 distance to waypoint is: %.1f",sqrt(pow(local_pos_1.pose.pose.position.x-pose_1.pose.position.x,2)+pow(local_pos_1.pose.pose.position.y-pose_1.pose.position.y,2)+pow(local_pos_1.pose.pose.position.z-pose_1.pose.position.z,2)) );
